...
function initialiseTicTacToe(context: SandboxContext) {
  ...
  const render = () => {
    ...
    // Draw the circle
    const circlePosition = params.circlePosition ?? { x: 0.5, y: 0.5 };
    const circleX = circlePosition.x * width;
    const circleY = circlePosition.y * height;
    const circleRadius = params.circleRadius ?? 50;
    const circleColor = params.circleColor ?? "#ffffff";
